<?php
    ...
    switch($_GET["op"]){
        ...
        break;
        case "listar2":
            $datos=$documentooc->get_documentooc_x_ticket_soloAgentes($_POST["tickoc_id"]);
            $data= Array();
            foreach($datos as $row){
                $sub_array = array();
                $url = '../../public/filesoc/' . $row["tickocd_id"] . '/' . $row["dococ_nom"];
                $sub_array[] = '<a href="../../public/filesoc/'.$_POST["tickocd_id"].'/'.$row["dococ_nom"].'" target="_blank">'.$row["dococ_nom"].'</a>';
                //$sub_array[] = '<a type="button" href="../../public/filesoc/'.$_POST["tickocd_id"].'/'.$row["dococ_nom"].'" target="_blank" class="btn btn-inline btn-primary btn-sm ladda-button"><i class="fa fa-eye"></i></a>';
                $sub_array[] = '<a type="button" href="' . $url . '" target="_blank" class="btn btn-inline btn-primary btn-sm ladda-button"><i class="fa fa-eye"></i></a>';
                $data[] = $sub_array;
            }
